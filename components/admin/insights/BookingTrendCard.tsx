...
export function BookingTrendCard() {
  ...
  const TrendIcon = diff > 0 ? TrendingUpIcon : diff < 0 ? TrendingDownIcon : MinusIcon
  const isPositive = diff > 0
  const isNegative = diff < 0

  return (
    <Card className="relative overflow-hidden bg-linear-to-br from-amber-500 via-amber-500 to-orange-500 border-0 shadow-lg shadow-amber-500/25">
      <div className="absolute top-0 right-0 size-40 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/4" />
      <div className="absolute bottom-0 left-0 size-24 bg-white/5 rounded-full translate-y-1/2 -translate-x-1/4" />
