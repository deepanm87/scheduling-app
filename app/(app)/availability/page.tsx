...
export default async function AvailabilityPage() {
  ...
  const initialBusyBlocks: BusyBlock[] = busyTimes
    .filter(
      busy => !isOverlappingWithBooking(new Date(busy.start), new Date(busy.end))
    )
    .map((busy, index) => ({
      id: `busy-${index}`,
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Drag to create time blocks. Click a block to remove it. Changes save automatically.
          </p>
          <div className="mt-2 flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
            {bookedBlocks.length > 0 && (
              <>
                <span className="flex items-center gap-1">
                  <span className="inline-block size-3 bg-green-600 rounded" />
                  Accepted
                </span>
                <span className="flex items-center gap-1">
                  <span className="inline-block size-3 bg-amber-500 rounded" />
                  Tentative
                </span>
                <span className="flex items-center gap-1">
                  <span className="inline-block size-3 bg-red-500 rounded" />
                  Declined
                </span>
                <span className="flex items-center gap-1">
                  <span className="inline-block size-3 bg-gray-500 rounded" />
                  Pending
                </span>
              </>
            )}
            {initialBusyBlocks.length > 0 && (
              <span className="flex items-center gap-1">
                <span className="inline-block size-3 bg-red-200 border border-red-400 rounded" />
                Busy (external)
              </span>
            )}
          </div>
